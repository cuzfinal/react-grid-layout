...
export function calcGridColWidth (positionParams: PositionParams): number {
  // containerPadding 整个容器的内边距
  // containerWidth 整个容器的宽度
  // cols 总列数 
  // margin 表示的是每个格子的外边距
  const { margin, containerPadding, containerWidth, cols } = positionParams;
  // containerWidth = colWidth * cols + (margin * (cols - 1)) + (containerPadding * 2)
  // colWidth * cols = containerWidth - (margin * (cols - 1)) - (containerPadding * 2)
...
export function calcXY(
  ...
  let x = (left - containerPadding[0]) / (colWidth + margin[0]);
  let y = (top - containerPadding[1]) / (rowHeight + margin[1]);
